...
from steam_etl.ingestion import fetch_and_save_top500_games, fetch_steam_store_details
...
with DAG(
    ...
    fetch_and_save_games = PythonOperator(
        task_id="fetch_and_save_top500_games_to_local",
        python_callable=fetch_and_save_top500_games,
    )
    ...
    upload_steamspy_to_gcs = PythonOperator(
        task_id="upload_steamspy_top500_to_gcs",
        python_callable=upload_file_to_gcs,
        op_kwargs={
            "src": "{{ ti.xcom_pull(key='return_value', task_ids='fetch_and_save_top500_games_to_local') | replace('steamspy_top500_games_','/opt/airflow/data/steamspy_top500_games_') }}",
            "dst": "bronze/SteamSpy/{{ ds }}/{{ ti.xcom_pull(key='return_value', task_ids='fetch_and_save_top500_games_to_local') }}",
